...
const AIAnalysis: React.FC<AIAnalysisProps> = ({ candidates, totalVotes }) => {
  ...
  const freshTotalVotes: number = (freshCandidates as FreshCandidate[]).reduce(
  ...
  return (
    <div className="bg-white rounded-lg p-6 border shadow-sm flex flex-col">
      {/* Header */}
      ...
      >
        {/* ... contenu complet identique à avant ... */}
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-700 mb-4">
                  <span className="font-bold text-purple-600">{insights.leader.name}</span> leads with{' '}
                  <span className="font-bold text-lg text-purple-700">{getVotePercentage(leaderVotes)}%</span> of the votes
                </p>
          ...
          <div className="bg-pink-50 rounded-lg p-4 border border-pink-200">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-700 text-sm">
                  Confidence of <span className="font-bold text-pink-600">{insights.confidence}%</span> based on current trends.
                </p>
              </div>
            </div>
          </div>
      ...
      <button
        ...
        className="mt-6 self-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center space-x-2"
        aria-expanded={isOpen}
        aria-label={isOpen ? "Réduire la section AI Analysis" : "Développer la section AI Analysis"}
      >
        <span>{isOpen ? "Réduire" : "Développer"}</span>
